...
Validate with `go test ./...`, `go vet ./...`, and `make build`. For graphical changes, render both views and inspect the images. Exercise the actual Ebitengine renderer with `--demo --frames 240 --file artifacts/edited.json --capture artifacts/editor.png --report artifacts/editor-report.json`; check the JSON for any ERROR action entries. CLI output must remain usable without a display. Keep generated artifacts under ignored `artifacts/` and binaries under ignored `bin/`.
