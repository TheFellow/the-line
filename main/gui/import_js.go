...
func (g *game) chooseCSV(requests chan<- csvImport) {
	...
	changed = js.FuncOf(func(_ js.Value, _ []js.Value) any {
		...
		if file.Get("size").Int() > 2<<20 {
			requests <- csvImport{err: fmt.Errorf("CSV import is limited to 2 MiB")}
			dispose()
			return nil
		}
		...
		success = js.FuncOf(func(_ js.Value, args []js.Value) any {
			requests <- csvImport{data: args[0].String(), name: name}
			cleanup()
			return nil
		})
		failure = js.FuncOf(func(_ js.Value, _ []js.Value) any {
			requests <- csvImport{err: fmt.Errorf("could not read CSV file %q", name)}
			cleanup()
			return nil
		})
